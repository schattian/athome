...
func GetDATABASE_USER() (notifier string) {
	notifier = os.Getenv("POSTGRES_USER")
	if notifier == "postgres" && envconf.NotInDevelopment() {
		panic("default db notifier given on non-local env")
	}
	return
}
