...
func (s *Server) retrieveAttributeDatas(ctx context.Context, db *sqlx.DB, in *pbsemantic.RetrieveAttributeDatasRequest) (*pbsemantic.RetrieveAttributeDatasResponse, error) {
	atts, err := data.FindServiceProviderAttributeDatasByMatch(ctx, db, in.GetEntity())
	if err != nil {
		return nil, status.Errorf(xerrors.Internal, "FindServiceProviderAttributeDatasByMatch: %v", err)
	}
