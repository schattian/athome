...
func (s *Server) getAttributeSchemas(ctx context.Context, db *sqlx.DB, in *pbsemantic.RetrieveAttributeSchemasRequest) (*pbsemantic.RetrieveAttributeSchemasResponse, error) {
	rows, err := db.QueryxContext(ctx, `SELECT * FROM service_provider_attribute_schemas WHERE category_id = $1`, in.GetCategoryId())
	if err != nil {
	...
	for rows.Next() {
		att := &schema.ServiceProviderAttributeSchema{}
