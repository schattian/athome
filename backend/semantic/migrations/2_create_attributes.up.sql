...
CREATE TABLE IF NOT EXISTS service_provider_attribute_schemas(
    id serial primary key,
    name varchar(50) not null,
    category_id integer not null,
    value_type varchar(10) not null
...
CREATE TABLE IF NOT EXISTS service_provider_attribute_datas(
    id serial primary key,
    schema_id integer not null,
    user_id integer not null,
    entity_id integer not null,
...
ALTER TABLE ONLY service_provider_attribute_datas ADD CONSTRAINT uq_service_provider_attribute_datas UNIQUE(schema_id, entity_id, entity_table);
