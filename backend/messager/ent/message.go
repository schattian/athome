...
func StatusFromPb(in *pbmessager.Status) (cAt, rAt, sAt Time, err error) {
...
func MessageFromPb(in *pbmessager.Message) (*Message, error) {
	msg := &Message{
...
func (n *Message) ToPbStatus() (*pbmessager.Status, error) {
	status := &pbmessager.Status{}
	var err error
...
func (n *Message) ToPb() (*pbmessager.Message, error) {
	status, err := n.ToPbStatus()
	if err != nil {
		return nil, errors.Wrap(err, "ToPbStatus")
	}
	return &pbmessager.Message{
